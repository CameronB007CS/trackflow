...
export default function CreateModal({ defaultStatus, onClose, onCreate }) {
  const [form, setForm] = useState({ title: '', description: '', type: 'Task', priority: 'Medium', status: defaultStatus });
  ...
  const submit = async () => {
    ...
    try { await onCreate(form); } catch (err) { setError(err.message); }
  };
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: '1.5rem', width: 440, maxWidth: '95vw' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <textarea name="description" value={form.description} onChange={handle} placeholder="Add more details..." rows={3} />
        </div>
