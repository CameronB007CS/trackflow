...
export default function IssueCard({ issue, onDragStart, onClick }) {
  ...
  return (
    <div draggable onDragStart={onDragStart} onClick={onClick}
      style={{ background: '#fff', borderRadius: 8, border: '1px solid rgba(0,0,0,0.08)', padding: '10px 12px', marginBottom: 8, cursor: 'grab', transition: 'border-color 0.15s' }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div title={issue.priority} style={{ width: 8, height: 8, borderRadius: '50%', background: pColor, flexShrink: 0 }} />
        <span style={{ fontSize: 11, color: '#999' }}>{issue.priority}</span>
        {issue.assignee_initials && (
          <div title={issue.assignee_name} style={{ marginLeft: 'auto', width: 22, height: 22, borderRadius: '50%', background: issue.assignee_color + '22', color: issue.assignee_color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 600 }}>
            {issue.assignee_initials}
          </div>
        )}
      </div>
